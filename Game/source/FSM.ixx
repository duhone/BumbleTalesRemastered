 ...
 *
 */
export module CR.Game.FSM;

import CR.Game.Property;
...
export namespace CR::Utility {
	...
	class StateHandlerImpl : public StateHandlerBase {
	  public:
		...
		virtual void operator()() { (m_object->*m_func)(); }
	...
	class SimpleFSM {
	  public:
		SimpleFSM() : m_state(0) {
			std::fill_n(std::back_inserter(m_handlers), NumStates, (StateHandlerBase*)nullptr);
		...
		void operator++(int _postfix) {
			if(m_state < (NumStates - 1)) m_state++;
		}
		void operator--(int _postfix) {
		...
		void operator()() {
			if(m_handlers[m_state] != nullptr) (*m_handlers[m_state])();
		}
		void AddStateHandler(int _state, StateHandlerBase* _handler) {
			if(_state < (NumStates)) m_handlers[_state] = _handler;
		}
	...
	class FSM {
	  public:
		FSM() : m_state(-1), m_skipUpdate(false) { State.Init(this, &FSM::GetState, &FSM::SetState); }
		~FSM() {
		...
		void operator++(int) {
			if(m_state < (m_states.size() - 1)) GoToState(m_state + 1);
		}
		void operator--(int) {
			if(m_state > 0) GoToState(m_state - 1);
		}
		void Reset() { m_state = -1; }
		void operator()() {
			if(m_state == -1) return;
			int new_state = IState::UNCHANGED;
			if(!m_skipUpdate && m_states[m_state] != nullptr) new_state = m_states[m_state]->Process();
			m_skipUpdate = false;
			if(new_state != IState::UNCHANGED) GoToState(new_state);
		}
	  ...
	  private:
		...
		void GoToState(int _state) {
			if(m_state >= 0) m_states[m_state]->End();
			m_state      = _state;
