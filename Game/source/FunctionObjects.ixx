export module CR.Game.FunctionObjects;
...
export namespace CR::Utility {
	...
	*/
	...
	/*!
	    ...
	    \endverbatim
	*/
	/* cant use in modern c++
	template<class T, class S>
	class Select1st : public std::unary_function<std::pair<T, S>, T> {
	  public:
	    T operator()(std::pair<T, S> arg) { return arg.first; }
	};*/
	...
	*/
	...
	class Select2nd : public std::unary_function<std::pair<T, S>, S> {
	  public:
	    S operator()(std::pair<T, S> arg) { return arg.second; }
	};*/
	...
	class Pair2ndToVector : public std::unary_function<std::pair<T, S>, void> {
	  public:
	    ...
	    void operator()(std::pair<T, S> arg) { list.push_back(arg.second); }
	...
	void FillAll(T& list, V _value) {
		std::fill(list.begin(), list.end(), _value);
	};
